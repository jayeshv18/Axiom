...
the printable character '0' actually starts at decimal value 48 (or hex 0x30).
Because the ASCII numbers are perfectly sequential, we can convert any raw single-digit integer into its printable character just by adding the character '0' to it.
For example: 3 + '0' evaluates exactly to the ASCII character '3'.
*/
...
/*the CPU is going to hand us raw memory addresses. Reading a memory address like 753664 in base-10 is incredibly confusing; reading it as 0xB8000 in base-16 (hexadecimal) makes instant sense.*/
...
/* convert the number to its hexadecimal string representation */
    while(num>0){
        int last_digit=num%16; /*extract the last digit of the number by calculating the remainder when divided by 16*/
        if(last_digit<10){
            str[i++]=last_digit+'0'; /*convert the last digit to its ASCII character representation and store it in the string*/
        }else{
            str[i++]=last_digit-10+'A'; /*convert the last digit to its uppercase hexadecimal character representation and store it in the string*/
        }
        num/=16;
    }
/* reverse the string */
